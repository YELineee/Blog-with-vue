<template>
  <div>
    <div v-for="(blogItem, index) in blogData" :key="index" class="w-full h-full text-slate-100">
      <div class="text-3xl md:text-4xl pt-8">{{ blogItem.title }}</div>
      <div class="text-sm text-slate-100 mb-3 mt-1">date: {{ blogItem.date }}</div>
      <div v-for="(contentPart, contentIndex) in blogItem.content" :key="contentIndex"
        class="p-1 md:p-3 text-sm md:text-xl text-slate-100">{{ contentPart }}</div>
